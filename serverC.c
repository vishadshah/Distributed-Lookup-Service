...
int cc ;
    ...
    printf("The backendserverC is up and running on port 23811\n");
    while(1)
    {
        //flag=1;
	//printf("\nWaiting to receive Requests\n");
        ...
        printf("The backendserverC received %s from the client\n", buffer);
 ...
 if(strncmp(buffer,search,6)==0)
   	{       
		...
		//char zerostring[256] = "Nothing Found";
		char * zerostring = (char *)malloc(sizeof(char)*4096); 
    		strcpy(zerostring,"Nothing Found");
		//printf("\nLength %d",strlen(zerostring));
                if(count == 0) sendto(sock,(void *)zerostring,strlen(zerostring),0,(struct sockaddr*) &servaddr,slen);
         }
	...
	{        cc = 1;
		...
		printf("The ServerC has sent %d matches\n",count);
	}
 else
	{
	
	}
	printf("The serverC has finished sending output to AWS\n");
