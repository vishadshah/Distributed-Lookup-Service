...
int cc ;
    ...
    printf("The backendserverA is up and running on port 21811\n");
    while(1)
    {
        //flag=1;
	//printf("Waiting to receive request\n");
        ...
        printf("The backendserverA received %s from the client\n", buffer);
 ...
 if(strncmp(buffer,search,6)==0)
   	{       
	        ...
	        char temp1[256];
       		const char s[2] = ":";
       		bzero(temp1,256);
       		FILE *filename=fopen("backendA.txt","r");
		count=0;
	...
	{        cc = 1;
		...
		printf("The ServerA has sent %d matches\n",count);
	}
 else
	{
	//printf("error\n");
...
printf("The serverA has finished sending output to AWS\n");
